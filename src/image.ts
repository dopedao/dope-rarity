...
export function svgFromItems(
  ...
  while (items.length) {
    ...
    if (item.length > 38) {
      const offset = 38 + item.substr(38).indexOf(" ")
      svg += (
        `<text x="10" y="${20 * (index + 1)}" fill="${color}">` + item.substr(0, offset) + `</text>`
      );
      svg += (
        `<text x="30" y="${20 * (index + 2)}" fill="${color}">` + item.substr(offset) + level + `</text>`
      );
      index += 2
      continue
    }
    ...
    svg += (
      `<text x="10" y="${20 * (index + 1)}" fill="${color}">` + item + level + `</text>`
    );
    index++
  }
